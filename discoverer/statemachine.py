...
class Statemachine(object):
    ...
    def dump(self, file=""):
        ...
        for t in self.__transitions:
            ...
            if t[3]=='client':
                print '{0} -> {1} [color=red,fontsize=10,label="{2}...",penwidth={3}]'.format(t[0],t[2],s,t[4] if self.__config.weightEdges else 1)
            else:
                print '{0} -> {1} [color=green,fontsize=10,label="{2}...",penwidth={3}]'.format(t[0],t[2],s,t[4] if self.__config.weightEdges else 1)
        print "}"
        if not file=="":
            handle.close()         
