...
class ClusterCollection():
    ...
    def fix_tokenization_errors(self, config):
        ...
        for cluster in clusters:
            ...
            lastVal = indices[len(indices)-1]
            for idx in range(len(indices)-1,-1,-1):
                #if not indices[idx]==indices[lowIdx]+1 or idx==0:
                if not indices[idx]==lastVal-1 or idx==0:
                    # End of sequence found
                    if abs(idx-lowIdx)>0:
                        print "Subsequence found: [{}-{}]".format(idx+1,lowIdx)
                        # Join tokens
                        if not idx+1 == lowIdx: # Is it a sequence of at least length 2
                            while lowIdx>idx+1:
