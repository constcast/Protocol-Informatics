## {{{ http://code.activestate.com/recipes/577361/ (r1)
import collections
class peekable(object):
    """ An iterator that supports a peek operation.
   
    this is a merge of example 19.18 of python cookbook part 2, peek
ahead more steps
    and the simpler example 16.7, which peeks ahead one step and stores
it in
    the self.preview variable.
   
    Adapted so the peek function never raises an error, but gives the
    self.sentinel value in order to identify the exhaustion of the iter
object.
    ...
    def _fillcache(self, n):
        """fill _cache of items to come, with one extra for the preview
variable
        """
        if n is None:
            n = 1
    ...
    def next(self, n=None):
        ...
        raises StopIteration if the iter is exhausted (self.sentinel is
found),
    ...
    def peek(self, n=None):
        """gives next item, without exhausting the iter, or a list of 0
or more next items
       
        with n == None, you can also use the self.preview variable,
which is the first item
        ...
        return result
## end of http://code.activestate.com/recipes/577361/ }}}
